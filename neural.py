...
class SymmetricalNeuralNetwork:
    ...
    def compute(self, input):
        assert input.shape == tuple((self.weights.shape[1] - 1,)), f"wrong input dimension, required dimensions are {
    ...
    def apply_symetry(self, weights_mat):
        for i in range(self._symmetry_mat.max() + 1):
    ...
    def mutable_weights(self):
        return np.logical_or(self._symmetry_mat % 1 != 0, self._symmetry_mat > 0)
